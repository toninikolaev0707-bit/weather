...
float snow(vec2 p, float jitter, vec2 direction, float speed, float size) {
  ...
  float scale = dot(minPoint, vec2(0.3, 0.3)) - 0.2 - size;
...
half4 main(float2 coord) {
  ...
  alpha += snow(p * 5.0 * tiling, 8.363, direction, 0.884 * baseSpeed, 0.08);
  if(density > 3) {
    alpha += snow(p * 35.0 * tiling, 110.683, direction, 1.24 * baseSpeed, 0.25);
  } else if(density > 2) {
    alpha += snow(p * 30.0 * tiling, 110.683, direction, 0.824 * baseSpeed, 0.25);
  } else if(density > 1) {
    alpha += snow(p * 15.0 * tiling, 110.683, direction, 0.824 * baseSpeed, 0.1);
  } else {
    alpha += snow(p * 5.0 * tiling, 110.683, direction, 0.524 * baseSpeed, 0.04);
  }

  if(density > 2) {
    float intensity = density > 3 ? 0.5 : 0.1;
    alpha += mist(p * 5.0 * tiling, baseSpeed) * intensity;
  }

  return vec4(0.8, 0.87, 0.87, 1.0) * alpha * 1;
}